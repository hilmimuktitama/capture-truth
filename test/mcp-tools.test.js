...
test("MCP exposes exactly capture tools and rejects local raw profile", async () => {
  assert.deepEqual(listCaptureTools().map((tool) => tool.name), ["capture.normalize", "capture.evidence_pack", "capture.candidate_review", "capture.doctor"]);
...
test("MCP JSON preserves metadata and derivation markers while excluding raw prose", async () => {
  ...
  const output = JSON.parse(result.content[0].text); assert.equal(result.content[0].text.includes("EXACT CONFIDENTIAL PROSE"), false); assert.equal(output.sources[0].metadata.owner, "ops"); assert.equal(output.candidate_claims.some((claim) => claim.derivation_version === "0.5.0" && claim.source_material === "metadata"), true);
});

test("MCP internal profile is the documented review surface for structured and metadata candidates", async () => {
  const result = await callCaptureTool("capture.evidence_pack", { sources: [{ id: "mcp-review-surface", locator: "mcp", raw: "PRIVATE RAW", fields: { status: "Ready" }, metadata: { owner: "ops" } }], profile: "internal-evidence-pack" });
